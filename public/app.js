...
function sendPcmChunk(
  ...
  if (triggered) {
    ...
    if (
      ...
      debugLog(
        `TRIGGER rms=${rms.toFixed(5)} speaking=${isAgentSpeaking}`
      );
...
async function startSdkSession(
  ...
  updateStatus(
    "Connecting via Conversation.startSession()...",
    "default"
  );
  ...
  ws.onmessage =
    (event) => {
      try {
        ...
        debugLog(
          `EVENT=${msg.type || Object.keys(msg).join(",")}`
        );
...
async function stopTranslator() {
  ...
  if (conversation) {
    try {
      if (
        typeof conversation.end ===
        "function"
      ) {
        await conversation.end();
      } else if (
        typeof conversation.endSession ===
        "function"
      ) {
